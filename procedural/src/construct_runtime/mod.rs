...
pub fn construct_runtime_modified(input: TokenStream) -> TokenStream {
...
fn decl_all_pallets<'a>(
    ...
    let all_pallets_without_system = names_by_feature.iter().map(|(feature_set, names)| {
        ...
        let test_cfg = feature_set
            .remove(&&&"test")
            .then_some(quote!(test))
            .into_iter();
    ...
    let system_pallet = match names_by_feature[&Vec::new()]
        .iter()
        .find(|n| **n == SYSTEM_PALLET_NAME)
    {
        Some(name) => name,
        None => {
