...
//! Implementation of `construct_runtime_modified`.
//!
//! `construct_runtime_modified` implementation is recursive and can generate code which will call itself in
...
//!
//! The `construct_runtime_modified` transitions from the implicit definition to the explicit one.
...
//! ```
//!
//! When all pallet parts are implicit, then the `construct_runtime_modified!` macro expands to its final
//! state, the `ExplicitExpanded`. Otherwise, all implicit parts are converted to an explicit
//! expanded part allow the `construct_runtime_modified!` to expand any remaining explicit parts to an
//! explicit expanded part.
//!
//! # Implicit to Explicit
//!
//! The `construct_runtime_modified` macro transforms the implicit declaration of each pallet
...
//!  - target: This is the `TokenStream` that contains the `construct_runtime_modified!` macro.
//!  - pattern: The pattern to match against in the target stream.
//!  - tokens: The tokens to added after the pattern match.
//!
//! The `construct_runtime_modified` macro uses the `tt_call` to get the default pallet parts via
...
//! The `match_and_replace` then will modify the the `construct_runtime_modified!` to expand the implicit
...
//! ```
//!
//! Visualizing the entire flow of `construct_runtime_modified!`, it would look like the following:
//!
//! ```ignore
//! +--------------------+     +---------------------+     +-------------------+
//! |                    |     | (defined in pallet) |     |                   |
//! | construct_runtime_modified! | --> |  tt_default_parts!  | --> | match_and_insert! |
//! | w/ no pallet parts |     |                     |     |                   |
//! +--------------------+     +---------------------+     +-------------------+
//!
//!     +--------------------+
...
/// Implementation of `construct_runtime_modified` macro. Either expand to some code which will call
/// `construct_runtime_modified` again, or expand to the final runtime definition.
pub fn construct_runtime_modified(input: TokenStream) -> TokenStream {
    ...
    let res = expander::Expander::new("construct_runtime_modified")
...
fn construct_runtime_implicit_to_explicit(
    ...
    let mut expansion =
        quote::quote!(new_modified_construct_runtime::construct_runtime_modified! { #input });
...
fn construct_runtime_final_expansion(
    ...
    let hidden_crate_name = "construct_runtime_modified";
    ...
    let warning = where_section.map_or(None, |where_section| {
        Some(
            proc_macro_warning::Warning::new_deprecated("WhereSection")
                .old("use a `where` clause in `construct_runtime_modified`")
                .new(
...
pub(crate) fn two128_str(s: &str) -> TokenStream2 {
    bytes_to_array(sp_crypto_hashing::twox_128(s.as_bytes()).into_iter())
}

pub(crate) fn bytes_to_array(bytes: impl IntoIterator<Item = u8>) -> TokenStream2 {
