...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        #[pallet::weight({
			...
			(
				  <T as pallet::Config>::WeightInfo::as_multi_threshold_1(call.using_encoded(|c| c.len() as u32))
        ...
        pub fn as_multi_threshold_1(
            origin: OriginFor<T>,
            ips_id: <T as pallet_assets::Config>::AssetId,
            call: Box<<T as Config>::Call>,
        ) -> DispatchResultWithPostInfo {
            let who = ensure_signed(origin)?;
            ensure!(
                pallet_assets::Pallet::<T>::balance(ips_id, who)
                    > (pallet_assets::Pallet::<T>::total_supply(ips_id) / 2u32.into()),
                Error::<T>::NotEnoughVotes
            );

            let id = Self::multi_account_id(ips_id);
            ...
            result
                .map(|post_dispatch_info| {
                    post_dispatch_info
                        .actual_weight
                        .map(|actual_weight| {
                            <T as pallet::Config>::WeightInfo::as_multi_threshold_1(call_len as u32)
                ...
                .map_err(|err| match err.post_info.actual_weight {
                    Some(actual_weight) => {
                        let weight_used = <T as pallet::Config>::WeightInfo::as_multi_threshold_1(
                            call_len as u32,
                        )
                        .saturating_add(actual_weight);
	...
	100000 //TODO
		...
		let s = 10u32; //TODO
			      ...
			      <T as pallet::Config>::WeightInfo::approve_as_multi_create(s)
				        .max(<T as pallet::Config>::WeightInfo::approve_as_multi_approve(s))
				        .max(<T as pallet::Config>::WeightInfo::approve_as_multi_complete(s))
        ...
        /// # </weight>
        #[pallet::weight(<T as pallet::Config>::WeightInfo::cancel_as_multi(10u32))] //TODO
        pub fn cancel_as_multi(
            ...
            let err_amount = <T as pallet::Config>::Currency::unreserve(&m.depositor, m.deposit);
...
impl<T: Config> Pallet<T> {
    ...
    fn operate(
        ...
        if let Some(mut m) = <Multisigs<T>>::get(&id, call_hash) {
            ...
            let mut approvals: <T as pallet_assets::Config>::Balance = 0u32.into();
            m.approvals
                .iter()
                .for_each(|acc| approvals += pallet_assets::Pallet::<T>::balance(ips_id, acc));
            // We only bother with the approval if we're below threshold.
            let maybe_pos = m.approvals.binary_search(&who).err().filter(|_| {
                approvals < (pallet_assets::Pallet::<T>::total_supply(ips_id) / 2u32.into())
            });
            ...
            let maybe_approved_call =
                if approvals >= (pallet_assets::Pallet::<T>::total_supply(ips_id) / 2u32.into()) {
            ...
            if let Some((call, call_len)) = maybe_approved_call {
                ...
                <Multisigs<T>>::remove(&id, call_hash);
                Self::clear_call(&call_hash);
                <T as pallet::Config>::Currency::unreserve(&m.depositor, m.deposit);
                ...
                Ok(get_result_weight(result)
                    .map(|actual_weight| {
                        <T as pallet::Config>::WeightInfo::as_multi_complete(10u32, call_len as u32) //TODO
                            .saturating_add(actual_weight)
                    })
                    .into())
            } else {
                ...
                let final_weight = if stored {
                    <T as pallet::Config>::WeightInfo::as_multi_approve_store(
                        10u32,
                        call_len as u32,
                    ) //TODO
                } else {
                    <T as pallet::Config>::WeightInfo::as_multi_approve(10u32, call_len as u32)
                    //TODO
                };
            ...
            let deposit = T::DepositBase::get() + T::DepositFactor::get() * 50u32.into(); // TODO: (pallet_assets::Pallet::<T>::total_supply(ips_id) / 2u32.into())
            ...
            let final_weight = if stored {
                <T as pallet::Config>::WeightInfo::as_multi_create_store(10u32, call_len as u32)
            //TODO
            } else {
                <T as pallet::Config>::WeightInfo::as_multi_create(10u32, call_len as u32)
                //TODO
            };
            // Call is not made, so the actual weight does not include call
            Ok(Some(final_weight).into())
